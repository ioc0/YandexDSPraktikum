...
def filter_by_genre(data,genre):
    ...
    return result
def print_by_genre(data,column,reverse):
    data.sort(key=lambda row: row[column], reverse=reverse)#сортирует по колонке
    print('Название                            | Год  | Рейтинг | Длина | Бюджет | Сборы  |')
    print('--------------------------------------------------------------------------------')
    for row in data[:5]:
        print('{: <35} | {} | {: >7.2f} | {: >5} | {: >6.1f} | {: >6.1f} |'.format(
            row[0], row[1], row[2], row[3], row[5], row[6]))
new_data = filter_by_genre(oscar_data,'криминал')
print_by_genre(new_data,6,True)
