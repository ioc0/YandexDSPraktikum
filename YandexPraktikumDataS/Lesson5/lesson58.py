'''.
Напишите функцию filter_suspense(), которая находит фильмы самых напряжённых жанров: «триллер» или «ужасы».
 Функция должна принимать как аргумент исходную таблицу и, не меняя её, возвращать отфильтрованную.'''
...
def filter_suspense(data):
    result = []
    for row in data:
        if 'триллер' in row[4] or 'ужасы' in row[4]:
            result.append(row)
    return result
